...
export interface Topic {
	...
	aliases: { repoId: string; enumName: string; member: string; source: SourceRef }[];
...
export interface Endpoint {
	...
	guards: string[];
	auth: boolean;
	requestSchemas: { location: 'body' | 'query' | 'param' | 'header'; type: string; schemaId: string | null }[];
...
export interface RepoDoc {
	...
	github: string;
	branch: string;
	commit: { sha: string; shortSha: string; date: string; author: string; subject: string };
	latestTag: string | null;
	packageVersion: string | null;
	nodeVersion: string | null;
	recentTags: { tag: string; date: string; sha: string }[];
	newCommitsSinceLastBuild: { sha: string; shortSha: string; date: string; author: string; subject: string }[];
	previousBuildCommit: string | null;
	readme: string | null;
	envVars: { name: string; comment?: string }[];
	domains: { name: string; useCases: number; endpoints: number; consumers: number }[];
	stats: RepoStats;
}
