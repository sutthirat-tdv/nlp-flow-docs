...
import {
  attachCollectionUsage,
  extractMongoCollections,
} from "./extract-collections.js";
...
function classifyByPath(
  file: string,
  className: string,
): { kind: DependencyKind; system?: string } {
  const system = SYSTEM_BY_PATH.find(([re]) => re.test(`/${file}`))?.[1];

  if (
    /\.use-case\.ts$|use-case\.ts$|\.use\.case\.ts$/.test(file) ||
    /UseCase$/.test(className)
  ) {
  ...
  if (/CacheService$|RedisService$/.test(className))
    return { kind: "cache", system: "redis" };
  if (
    /Logger|Metric|GracefulShutdown|FlushSummaryLog|MessageContextService/.test(
      className,
    )
  ) {
    return { kind: "logging" };
...
function domainFromPath(repo: RepoConfig, file: string): string {
  ...
  if (parts[1] === "controllers" && parts[2]) {
    return parts[3] && parts[2] !== "healthcheck"
      ? `${parts[2]}/${parts[3]}`
      : parts[2];
  }
  ...
  if (parts[1] === "loyaltyManagement") {
    ...
    if (group === "externalServices" && parts[3])
      return `externalServices/${parts[3]}`;
    return group;
  }
  if (parts[1] === "jobs" && parts[2]) return parts[2];
  if (parts[1] === "infrastructure" && parts[2])
    return `infrastructure/${parts[2]}`;
  return parts[1] ?? "root";
}
    ...
    switch (name) {
      ...
      default:
        if (name.startsWith("Is") || name.startsWith("Validate"))
          rules.push(toTitle(name));
    }
  }
  return [...new Set(rules)];
}

function apiPropertyMeta(decorators: string[]): {
  description?: string;
  example?: string;
  required?: boolean;
} {
  const api = decorators.find(
    (d) => d.startsWith("@ApiProperty") || d.startsWith("@ApiPropertyOptional"),
  );
  if (!api) return {};
  const description = /description:\s*(['"`])([\s\S]*?)\1/.exec(api)?.[2];
  const example =
    /example:\s*(['"`])([\s\S]*?)\1/.exec(api)?.[2] ??
    /example:\s*([^,}\n]+)/.exec(api)?.[1]?.trim();
...
function schemaLayer(file: string, name: string): SchemaLayer {
  ...
  if (/use-case\.dto\.ts$|dtos\/useCases?\/|useCases?\/dtos\//.test(file))
    return "use-case";
  if (/\.consumer\.dto\.ts$|dtos\/consumers?\//.test(file))
    return "kafka-event";
  if (/\.service\.dto\.ts$|dtos\/services?\//.test(file)) return "service";
  if (
    /entities|\.interface\.ts$|persistences|infrastructure\/database/.test(file)
  )
    return "persistence";
  if (/d03|d64|sap|pns|ikm|externals|repositories/.test(file))
    return "downstream";
...
export function extractRepo(
  ...
  const registerTopic = (
    ...
    if (!list.some((a) => a.enumName === enumName && a.member === member)) {
      list.push({
        enumName,
        member,
        source: sourceRef(repo, prov, file, line),
      });
    }
    ...
    node.forEachChild((child) => {
      ...
      if (ts.isEnumDeclaration(child)) {
        ...
        for (const member of child.members) {
          ...
          if (qualified !== enumName)
            registerTopic(value, qualified, memberName, file.path, line);
        }
      }
    });
  };
  ...
  const resolveTopicFromText = (text: string): string[] => {
    ...
    for (const m of text.matchAll(
      /(['"`])([a-z][A-Za-z0-9]*(?:\.[A-Za-z0-9_-]+){1,5})\1/g,
    )) {
      if (
        TOPIC_PATTERN.test(m[2]) &&
        /\.(pty|bff|cdc|cmd)\.|^mfaf\.|^esb\.|^sid\./.test(m[2])
      ) {
        topics.add(m[2]);
      }
    }
  ...
  const publishedIn = (text: string): string[] => {
    ...
    while ((match = PRODUCER_CALL_PATTERN.exec(text))) {
      ...
      for (const topic of resolveTopicFromText(balancedArgs(text, open)))
        topics.add(topic);
    }
    return [...topics];
  };
  ...
  for (const rel of files) {
    ...
    for (const cls of exportedClasses(file)) {
      ...
      for (const m of methodsOf(cls))
        methodText.set(methodName(m, file), textOf(file, m));
      classIndex.set(name, {
        name,
        file: rel,
    ...
    file.source.forEachChild((node) => {
      if (ts.isClassDeclaration(node) && node.name) {
        ...
        for (const member of node.members) {
          ...
          const decorators = getDecorators(member, file).map((d) =>
            d.text.replace(/\s+/g, " "),
          );
        ...
        addSchema({
          ...
          enumMembers: node.members.map((m) => ({
            ...
            value: m.initializer
              ? textOf(file, m.initializer).replace(/['"]/g, "")
              : "",
          })),
          source: sourceRef(repo, prov, rel, lineOf(file, node)),
          usedBy: [],
  ...
  const collections = extractMongoCollections({
    ...
    schemaIdFor,
    schemaFields: (id) =>
      id ? (schemaByName.get(id.split(":")[1] ?? "")?.fields ?? []) : [],
  ...
  for (const rel of files.filter(isUseCaseFile)) {
    ...
    for (const cls of exportedClasses(file)) {
      ...
      const entry =
        ...
        methods.find((m) =>
          /^(handle|run|process)$/.test(methodName(m, file)),
        ) ??
        methods[0];
      ...
      const inputType = entry?.parameters[0]?.type
        ...
        : null;
      const outputType = entry?.type
        ? unwrapType(textOf(file, entry.type))
        : null;

      const deps = constructorParams(cls, file).map(dependencyOf);
      const reach = reachOf(className, entryName, 0, new Set());
      const closure = {
        topics: new Set(reach.topics),
        systems: new Set(reach.systems),
      };
      ...
      for (const dep of deps) {
        if (dep.system && dep.kind !== "kafka-producer")
          closure.systems.add(dep.system);
      }
      ...
      const errors = [
        ...new Set(
          [
            ...classText.matchAll(
              /throw new ([A-Za-z0-9_]+)\(([\s\S]{0,120}?)\)/g,
            ),
          ].map(
            (m) => `${m[1]}(${m[2].replace(/\s+/g, " ").trim().slice(0, 90)})`,
          ),
        ),
      ].slice(0, 12);
      ...
      const calls = [
        ...new Set(
          thisCalls(classText)
            ...
            .filter(
              (t): t is string => !!t && /UseCase$|Manager$|Service$/.test(t),
            ),
        ),
      ];
  ...
  for (const rel of controllerFiles) {
    ...
    for (const cls of exportedClasses(file)) {
      ...
      for (const method of methodsOf(cls)) {
        ...
        const httpDecorator = decorators.find(
          (d) => HTTP_METHOD_DECORATORS[d.name],
        );
        if (!httpDecorator) continue;
        const routePath = (httpDecorator.args[0] ?? "").replace(/['"`]/g, "");
        const fullPath =
          `/${[...segments, routePath].filter(Boolean).join("/")}`.replace(
            /\/+/g,
            "/",
          );
        ...
        for (const param of method.parameters) {
          ...
          for (const d of paramDecorators) {
            ...
            else if (/^(Param|Params|ValidateParams)$/.test(d.name))
              location = "param";
            else if (/^(Headers|ValidateHeader)$/.test(d.name))
              location = "header";
            if (!location) continue;
            const dtoFromDecorator =
              d.args[0] && /^[A-Z]/.test(d.args[0]) ? d.args[0] : null;
            const type = (
              dtoFromDecorator ??
              unwrapType(typeText) ??
              ""
        ...
        for (const d of decorators) {
          if (
            !/^Api(Response|OkResponse|CreatedResponse|BadRequestResponse|ConflictResponse|NotFoundResponse|UnauthorizedResponse|InternalServerErrorResponse)$/.test(
              d.name,
            )
          ) {
            continue;
        ...
        if (method.type) {
          ...
          if (
            type &&
            schemaIdFor(type) &&
            !responseSchemas.some((r) => r.type === type)
          ) {
            responseSchemas.unshift({
              status: "OK",
              type,
              schemaId: schemaIdFor(type),
            });
        ...
        for (const t of invokedUseCases)
          useCaseByClass.get(t)?.invokedBy.push(id);
      }
    }
  }
  ...
  for (const rel of commandFiles) {
    ...
    for (const cls of exportedClasses(file)) {
      ...
      for (const invoked of invokedNames) {
        ...
        if (/Logger|Metric|AppInsights|ConfigBatch|AxiosService/.test(invoked))
          continue;
        const entryClass = classIndex.get(invoked);
        if (!entryClass || !/UseCase$|Service$|Manager$/.test(invoked))
          continue;
        const called = thisCalls(methodText).find(
          (c) =>
            unwrapType(
              ctorDeps.find((d) => d.name === c.property)?.type ?? "",
            ) === invoked,
        );
  ...
  for (const rel of files) {
    ...
    const file = parse(rel);
    if (
      !/@EntryPoint\s*\(|@EventPattern\s*\(|@MessagePattern\s*\(/.test(
        file.text,
      )
    )
      continue;
    for (const cls of exportedClasses(file)) {
      ...
      for (const method of methodsOf(cls)) {
        ...
        for (const invoked of invokedUseCases) {
          ...
          if (
            entryClass.kind === "kafka-producer" ||
            /RequestReply|Logger|Metric/.test(invoked)
          ) {
            continue;
          }
          const called = thisCalls(methodText).find(
            (c) =>
              ctorDeps.find((d) => d.name === c.property)?.type === invoked,
          );
          const managerReach = called
            ? reachOf(invoked, called.method, 0, new Set())
            : {
          ...
          const uc: UseCase = {
            ...
            domain: domainFromPath(repo, entryClass.file),
            title: toTitle(
              called?.method ?? invoked.replace(/Manager$|Service$/, ""),
            ),
            source: sourceRef(repo, prov, entryClass.file, entryClass.line),
            loc: 0,
