...
const FACTORY_HTTP =
  /\bthis\.(getMany|get|post|put|patch|delete)(?![A-Za-z0-9_])\s*(?:<[^>]*>)?\s*\(/g;
const AXIOS_HTTP =
  /\bthis\.axios\.(get|post|put|patch|delete)(?![A-Za-z0-9_])\s*(?:<[^>]*>)?\s*\(/g;
...
function isWrapperClass(className: string, file: string): boolean {
  ...
  if (/Factory$|FactoryService$|FactoryRepository$/.test(className))
    return true;
...
function usesAxios(cls: ts.ClassDeclaration, file: ParsedFile): boolean {
  ...
  const heritage =
    cls.heritageClauses?.map((h) => textOf(file, h)).join(" ") ?? "";
...
function fragmentFromExpr(
  ...
  if (
    /^[A-Za-z0-9_$]+(?:\.[A-Za-z0-9_$]+)+$/.test(expr) &&
  ...
  if (/^this\.[A-Za-z0-9_]+\s*\(/.test(expr)) {
    const arg = /this\.[A-Za-z0-9_]+\s*\(\s*([^)]*)\s*\)/
      .exec(expr)?.[1]
      ?.trim();
...
function pathFromArg(
  ...
  const ident =
    /^(?:const|let|var)?\s*([A-Za-z0-9_$]+)(?:\.toString\(\))?$/.exec(arg);
  ...
  const newUrl =
    /new\s+URL\s*\(\s*(?:path\.join\s*\(\s*this\.baseUrl\s*,\s*)?([^)]+)\)/.exec(
      arg,
    );
...
function callsIn(
  ...
  const collect = (
    re: RegExp,
    via: HttpOperation["via"],
    verbOf: (name: string) => HttpVerb,
  ) => {
    re.lastIndex = 0;
    ...
    while ((match = re.exec(text))) {
      ...
      const path =
        pathFromArg(first, props, text, helpers, file, parse, fileSet) ||
        ":url";
...
export function extractHttpClients(args: ExtractHttpArgs): HttpClient[] {
  ...
  for (const rel of files) {
    if (
      !/\.ts$/.test(rel) ||
      /\.spec\.ts$|\.test\.ts$|\.d\.ts$|\.module\.ts$/.test(rel)
    )
      continue;
    if (!/infrastructure|externalServices|sharedModules|\/jobs\//.test(rel))
      continue;
    ...
    for (const cls of exportedClasses(file)) {
      ...
      for (const member of cls.members) {
        ...
        if (ts.isIdentifier(member.initializer)) {
          const value = resolveNameToString(
      ...
      for (const method of methodsOf(cls)) {
        ...
        const walk = (methodNameWalk: string, depth: number) => {
          ...
          for (const helper of selfCallsIn(body)) {
            if (helpers.has(helper) && helper !== methodNameWalk)
              walk(helper, depth + 1);
          }
        };
        walk(name, 0);
...
export function attachHttpUsage(
  ...
  for (const uc of useCases) {
    const into = new Map<
      string,
      Map<string, HttpAccess["operations"][number]>
    >();
    ...
    for (const a of access) {
      ...
      if (client && !client.usedByUseCaseIds.includes(uc.id))
        client.usedByUseCaseIds.push(uc.id);
    }
