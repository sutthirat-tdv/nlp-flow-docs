...
function classifyTopic(name: string): Topic['kind'] {
	...
	if (/(ed|Created|Updated|Deleted|Queried|Onboarded|Redeemed|Applied)$/.test(last)) return 'event';
	if (/^(create|update|delete|query|get|apply|adjust|redeem|check|process|onboard|register|transfer|manage|generate|void|cancel|expire|import|export|upload|sync|send|calculate|validate|notify|reconcile|handle|refund|earn|burn|add|remove|suspend|resume|approve|reject|assign|revoke|count|list|search|verify|publish|produce|consume|run|start|stop|reset|recalculate|enrich|resolve|bulk)/.test(
			last,
		))
		return 'command';
	return 'unknown';
...
function loadTopicFamilies(snapshotRoot: string): Map<string, string> {
	...
	for (const raw of readFileSync(file, 'utf8').split('\n')) {
		...
		if (itemMatch && currentKey) members.push(itemMatch[1].replace(/['"]/g, ''));
	}
	flush();
	return families;
}
...
function main(): void {
	...
	for (const repo of config.repos) {
		...
		if (!prov) throw new Error(`No snapshot for ${repo.name}. Re-run \`npm run sync\`.`);
		...
		for (const [topic, family] of loadTopicFamilies(root)) topicFamilies.set(topic, family);
		console.log(
	...
	for (const consumer of consumers) ensureTopic(consumer.topic).consumedBy.push(consumer.id);
	for (const uc of useCases) {
		for (const name of uc.producesTopics) ensureTopic(name).producedBy.push(uc.id);
	}
	...
	const topics = [...topicMap.values()]
		.filter(t => t.consumedBy.length || t.producedBy.length || t.aliases.length)
	...
	for (const schema of schemas) {
		for (const field of schema.fields) {
			for (const ref of field.refs) {
				...
				if (target && target.id !== schema.id && !target.usedBy.includes(schema.id)) {
	...
	for (const repo of config.repos) {
		...
		const domains = [...domainNames]
			...
			.sort((a, b) => b.useCases + b.endpoints + b.consumers - (a.useCases + a.endpoints + a.consumers));
		...
		repoDocs.push({
			...
			stats: {
				...
				topicsProduced: new Set(extraction.useCases.flatMap(u => u.producesTopics)).size,
	...
	// ------------------------------------------------------------ systems
	const systems: DownstreamSystem[] = Object.entries(config.downstreamSystems).map(
		([id, cfg]) => {
	...
	console.log(`${flows.length} flows (${flows.filter(f => f.crossService).length} cross service)`);

	// Tag use cases so search and filters have something to bite on.
	for (const uc of useCases) {
		const tags = new Set(uc.tags);
		if (uc.invokedBy.some(id => id.includes('consume '))) tags.add('kafka-triggered');
		if (uc.invokedBy.some(id => /:(GET|POST|PUT|PATCH|DELETE|ANY) /.test(id))) tags.add('http-triggered');
