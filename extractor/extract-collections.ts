...
export interface ExtractCollectionsArgs {
  ...
  ) => { name: string; type: string; refs: string[] }[];
...
export function classifyOpKind(method: string, body: string): CollectionOpKind {
  ...
  if (drivers.includes("insertOne") || drivers.includes("insertMany"))
    return "create";
  ...
  if (
    /^(update|set|patch|replace|increment|decrement|merge|bind|unbind|atomic)/.test(
      n,
    )
  ) {
    return "update";
  ...
  if (
    /^(find|get|list|count|query|search|exists|has|lookup|retrieve|load|read|aggregate|cursor)/.test(
      n,
    )
  ) {
    return "read";
...
function collectionCallArgs(
  text: string,
): { entity: string | null; arg: string }[] {
  ...
  if (schemaFields.length) {
    return schemaFields.map((field) => ({
      ...
      fk:
        ...
        [...fkNames].some(
          (via) => via === field.name || via.startsWith(`${field.name}:`),
        ),
    }));
  }
...
export function extractMongoCollections(
  args: ExtractCollectionsArgs,
): MongoCollection[] {
  const {
    repo,
    prov,
  ...
  for (const rel of files) {
    if (
      !/\.mongo\.repository\.ts$/.test(rel) &&
      !/mongoDb\/.*repositories/.test(rel)
    )
      continue;
    ...
    for (const cls of exportedClasses(file)) {
      ...
      for (const member of cls.members) {
        ...
        const pname = ts.isIdentifier(member.name)
          ? member.name.text
        ...
        if (ts.isStringLiteral(member.initializer)) {
          ...
          collectionName = resolveNameToString(
      ...
      for (const [local, spec] of namedImports(file)) {
        ...
        if (value && value !== collectionName)
          importedCollectionConsts.push(value);
        void spec;
      }
  ...
  for (const coll of collections) {
    ...
    for (const field of fields) {
      ...
      for (const typeName of typeCandidates) {
        const target =
          byEntity.get(typeName) ?? byEntity.get(stripRef(typeName));
        if (!target || target.id === coll.id) continue;
        if (
          coll.related.some(
            (r) => r.collectionId === target.id && r.kind === "type",
          )
        )
          continue;
        coll.related.push({
    ...
    coll.related = coll.related.filter(
      (r) => byId.has(r.collectionId) || r.kind === "import",
    );
  ...
  for (const coll of collections) {
    ...
    for (const rel of coll.related) {
      if (
        !existing.related.some(
          (r) => r.collectionId === rel.collectionId && r.via === rel.via,
        )
      ) {
    ...
    if (!existing.fields.length && coll.fields.length)
      existing.fields = coll.fields;
  }
...
export function attachCollectionUsage(
  ...
  const kindOf = (coll: MongoCollection, method: string): CollectionOpKind =>
    coll.operations.find((o) => o.name === method)?.kind ??
    classifyOpKind(method, "");
  ...
  const walk = (
    ...
    for (const helper of selfCallsIn(text)) {
      if (entry.methodText.has(helper))
        walk(className, helper, depth + 1, seen, into);
    }
  };
  ...
  for (const uc of useCases) {
    ...
    const access: CollectionAccess[] = [...into.entries()].map(
      ([collectionId, ops]) => ({
        collectionId,
    ...
    for (const a of access) {
      ...
      if (coll && !coll.usedByUseCaseIds.includes(uc.id))
        coll.usedByUseCaseIds.push(uc.id);
    }
  ...
  for (const group of byName.values()) {
    ...
    for (const a of group) {
      for (const b of group) {
        ...
        if (
          a.related.some(
            (r) => r.collectionId === b.id && r.kind === "same-name",
          )
        )
          continue;
        a.related.push({
