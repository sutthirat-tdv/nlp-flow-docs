...
function Sidebar({ onSearch }: { onSearch: () => void }) {
	...
	const link = (to: string, label: string, count?: number) => (
		<NavLink key={to} to={to} className="sidebar__link" end={to === '/'}>
			<span>{label}</span>
			{count !== undefined ? <span className="sidebar__count">{count.toLocaleString()}</span> : null}
		</NavLink>
	);
	...
	return (
		<nav className="sidebar">
			...
			<div className="sidebar__tagline">Flows, use cases and data schemas · branch sit</div>
...
export function App() {
	...
	return (
		<div className="layout">
			...
			<main className="main">
				<div className="main__inner">
					<Routes>
						...
						<Route path="/dependencies/:clientId" element={<DependencyDetailPage />} />
			...
			{paletteOpen ? <SearchPalette onClose={() => setPaletteOpen(false)} /> : null}
		</div>
	);
}
...
function Footer() {
	...
	return (
		<div className="footer-note">
			<span>
				Generated from source on {new Date(core.generatedAt).toLocaleString()} · nothing here is
				hand maintained
			</span>
