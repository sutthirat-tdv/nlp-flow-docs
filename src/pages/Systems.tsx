  ...
  return (
    <>
      <PageHead title="Downstream systems">
        Everything the platform depends on beyond these four repositories. A use
        case is listed here when the call graph from its entry method reaches
        that system, so this doubles as a blast radius map: if D03 is down, the
        use cases under D03 are what breaks.
      </PageHead>
        ...
        >
          <div className="card">
            ...
            {system.kind === "http" ? (
              <p style={{ marginTop: 0 }}>
                <Link
                  to={`/dependencies?system=${encodeURIComponent(system.id)}`}
                >
              ...
              render={(id: string) => {
                ...
                return (
                  <div key={id} style={{ padding: "2px 0" }}>
                    <Link to={`/use-cases/${encodeURIComponent(id)}`}>
                      {useCase.title}
                    </Link>{" "}
