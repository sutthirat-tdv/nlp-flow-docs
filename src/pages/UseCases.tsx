...
export function UseCasesPage() {
  ...
  const domains = useMemo(() => {
    const set = new Set(
      core.useCases
        .filter((u) => repo === "all" || u.repoId === repo)
        .map((u) => u.domain),
    );
    return [...set].sort();
  }, [core.useCases, repo]);
  ...
  return (
    <>
      <PageHead title="Use cases">
        The unit of business logic in all four services: one class, one{" "}
        <code>execute()</code>. This is the fastest way to answer “where does
        this rule actually live?”.
      </PageHead>
      ...
      <div className="toolbar">
        ...
        <span className="dimmer" style={{ fontSize: 12.5, marginLeft: "auto" }}>
          {useCases.length.toLocaleString()} of{" "}
          {core.useCases.length.toLocaleString()}
        </span>
      </div>
        ...
        <table>
          ...
          <tbody>
            {useCases.slice(0, 400).map((useCase) => (
              <tr key={useCase.id}>
                <td>
                  <Link to={`/use-cases/${encodeURIComponent(useCase.id)}`}>
                    {useCase.title}
                  </Link>
      ...
      {useCases.length > 400 ? (
        <p className="dimmer" style={{ fontSize: 12.5 }}>
          Showing the first 400 of {useCases.length.toLocaleString()}. Narrow
          the filters or press ⌘K.
        </p>
      ) : null}
    </>
  );
...
function DependencyRow({ dep }: { dep: Dependency }) {
  ...
  const target = dep.targetId
    ? indexes.useCaseById.get(dep.targetId)
    : undefined;
  const collection = dep.targetId
    ? indexes.collectionById.get(dep.targetId)
    : undefined;
  const httpClient = dep.targetId
    ? indexes.httpClientById.get(dep.targetId)
    : undefined;
  return (
    <tr>
    ...
    <div className="card">
      ...
      {!schema ? (
        ...
        <span className="dimmer">
          No declared fields (inherits or is a primitive alias).
        </span>
      ) : (
        <div>
          ...
          {schema.fields.length > 8 ? (
            <div style={{ marginTop: 8 }}>
              <SchemaLink id={schemaId} /> has {schema.fields.length} fields in
              total.
            </div>
          ) : null}
        </div>
...
export function UseCaseDetailPage() {
  ...
  const relatedFlows = indexes.flowSummaryById
    ? [...indexes.flowSummaryById.values()].filter(
        (f) => f.entry && useCase.invokedBy.includes(f.entry.id),
      )
    : [];
  ...
  return (
    <>
      ...
      <Section
        ...
        subtitle={`${triggers.length} entry point(s)`}
      >
        {triggers.length === 0 ? (
          <Empty>
            Nothing in these repos calls it directly. It is either wired only
            through a module we could not resolve, called from another use case,
            or dead code worth checking.
          </Empty>
        ) : (
          <div className="table-wrap table-wrap--freeze">
            <table>
                        ...
                        {consumers.length === 0 ? (
                          ...
                          <div className="badges">
                            {consumers.map((c) => (
                              <Badge
                                key={c.id}
                                tone={c.isReplyListener ? undefined : "teal"}
                              >
                                {indexes.repoById.get(c.repoId)?.title}:{" "}
                                {c.controller}
      ...
      >
        {(useCase.collectionAccess ?? []).length === 0 ? (
          <Empty>
            ...
            <code>{useCase.entryMethod}()</code>. Loyalty writes live in TMF658;
            BFF collections are local read models.
          </Empty>
        ) : (
          <div className="table-wrap table-wrap--freeze">
            <table>
              ...
              <tbody>
                {(useCase.collectionAccess ?? []).map((access) => {
                  ...
                  const queries = access.operations.filter(
                    (o) => o.kind === "read",
                  );
      ...
      >
        {(useCase.httpAccess ?? []).length === 0 ? (
          <Empty>
            This use case does not call an Axios client from{" "}
            <code>{useCase.entryMethod}()</code>.
          </Empty>
        ) : (
          <div className="table-wrap table-wrap--freeze">
