	...
	return (
		<>
			<PageHead title="Services">
				Four repositories, three layers. Requests enter through a BFF, coarse business operations
				are aggregated, and the TMF658 domain service is the system of record. Everything between
				them is Kafka.
			</PageHead>
			...
			<Section title="Who talks to whom" subtitle="shared Kafka topics between each pair">
				<CrossServiceMatrix />
			</Section>
		</>
	);
	...
	const rows = useMemo(() => {
		...
		for (const producerRepo of core.repos) {
			for (const consumerRepo of core.repos) {
				...
				for (const useCase of core.useCases.filter(u => u.repoId === producerRepo.id)) {
					for (const topic of useCase.producesTopics) {
						const consumers = indexes.consumersByTopic.get(topic) ?? [];
						if (consumers.some(c => c.repoId === consumerRepo.id && !c.isReplyListener)) {
							topics.add(topic);
						}
					}
				}
				if (topics.size) {
					out.push({ from: producerRepo.id, to: consumerRepo.id, topics: [...topics].sort() });
				}
			}
		}
...
export function ServiceDetailPage() {
	...
	const producedTopics = [
		...new Set(core.useCases.filter(u => u.repoId === repo.id).flatMap(u => u.producesTopics)),
	].sort();
	...
	return (
		<>
			...
			<div className="grid grid--4">
				...
				/>
				<Stat value={repo.stats.useCases} label="Use cases" to={`/use-cases?repo=${repo.id}`} />
					...
					<KeyValue
						rows={[
							...
							[
								'Committed',
								<span className="dim">
									{new Date(repo.commit.date).toLocaleString()} by {repo.commit.author}
								</span>,
							],
							[
								'Nearest tag',
			...
			{repo.readme ? (
				<Section title="Repository README" subtitle="verbatim, first 4000 characters">
					<div className="card">
						<pre
							className="mono dim"
