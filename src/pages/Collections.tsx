...
function kindCounts(collection: MongoCollection) {
	const writes = collection.operations.filter(o => WRITE_KINDS.includes(o.kind)).length;
	const queries = collection.operations.filter(o => QUERY_KINDS.includes(o.kind)).length;
	const updates = collection.operations.filter(o => o.kind === 'update').length;
...
export function CollectionsPage() {
	...
	const collections = useMemo(() => {
		...
		return (core.collections ?? [])
			...
			.sort(
				(a, b) =>
					b.usedByUseCaseIds.length - a.usedByUseCaseIds.length || a.name.localeCompare(b.name),
			);
	...
	return (
		<>
			<PageHead title="Mongo collections">
				Every collection a <code>*MongoRepository</code> actually opens. Native driver, not
				Mongoose — the name is the string passed to <code>db.collection()</code>. Open one to see
				when documents are inserted, when they are queried, and which other collections a document
				points at.
			</PageHead>
					...
					<table>
						...
						<tbody>
							{collections.map(collection => {
								...
								const related = collection.related.filter(r => r.kind !== 'same-name');
								return (
									<tr key={collection.id}>
										<td>
											<Link className="mono" to={`/database/${encodeURIComponent(collection.id)}`}>
												{collection.name}
											</Link>
										...
										<td className="mono dim">{collection.usedByUseCaseIds.length}</td>
										<td>
											{related.length === 0 ? (
...
export function CollectionDetailPage() {
	...
	const related = collection.related
		.map(link => ({ link, target: indexes.collectionById.get(link.collectionId) }))
		.filter((row): row is { link: (typeof collection.related)[number]; target: MongoCollection } =>
			!!row.target,
		);
	...
	return (
		<>
			<PageHead
				...
				actions={<SourceLink source={collection.source} label="open in GitHub" />}
			>
				...
				. {counts.writes} create/upsert method{counts.writes === 1 ? '' : 's'}, {counts.queries}{' '}
				query method{counts.queries === 1 ? '' : 's'}.
			</PageHead>
			...
			<div className="card">
				<KeyValue
					rows={[
						...
						['Mongo collection', <span className="mono">{collection.name}</span>],
						[
							'Connection',
							collection.connection ? (
			...
			>
				<AccessTable rows={createdBy} empty="Nothing in these repos inserts into this collection." />
			</Section>
			...
			>
				<AccessTable rows={queriedBy} empty="Nothing in these repos reads this collection." />
			</Section>
					...
					<table>
						...
						<tbody>
							{collection.operations.map(op => (
								<tr key={op.name}>
									<td>
										<a className="mono" href={op.source.url} target="_blank" rel="noreferrer">
											{op.name}()
										</a>
									</td>
									<td>
										<Badge tone={KIND_TONE[op.kind]}>{KIND_LABEL[op.kind]}</Badge>
									</td>
			...
			>
				<div className="card">
					{!collection.entitySchemaId ? (
						<Empty>The repository does not declare a document type we could match to a schema.</Empty>
					) : !schema ? (
			...
			>
				{inRepo.length ? (
								...
								{inRepo.map(({ link, target }) => (
									<tr key={`${link.collectionId}:${link.via}`}>
										...
										<td>
											<Badge
												tone={
													link.kind === 'import' ? 'amber' : link.kind === 'type' ? 'teal' : 'accent'
												}
											>
												{link.kind === 'import'
					...
					<Empty>
						No field types or imported collection constants pointed at another collection in this
						service.
					</Empty>
				)}
			</Section>
				...
				>
					<div className="pill-list">
						{sameName.map(({ target }) => (
							<Badge key={target.id}>
								<CollectionLink id={target.id} />{' '}
								<span className="dimmer">· {indexes.repoById.get(target.repoId)?.title}</span>
							</Badge>
						))}
					</div>
				</Section>
...
function useCasesForKinds(
	...
	const rows: { useCaseId: string; operations: { name: string; kind: CollectionOpKind }[] }[] = [];
	for (const id of collection.usedByUseCaseIds) {
		const useCase = indexes.useCaseById.get(id);
		const access = (useCase?.collectionAccess ?? []).find(a => a.collectionId === collection.id);
		const operations = (access?.operations ?? []).filter(o => kinds.includes(o.kind));
...
function AccessTable({
	...
	rows: { useCaseId: string; operations: { name: string; kind: CollectionOpKind }[] }[];
	empty: string;
}) {
...
function CollectionField({
	...
	return (
		<div className="schema-field">
			...
			{field.comment ? <div className="schema-field__desc">{field.comment}</div> : null}
		</div>
	);
}
