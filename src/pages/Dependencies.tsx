...
export function DependenciesPage() {
	...
	const systems = useMemo(() => {
		const ids = new Set((core.httpClients ?? []).map(c => c.system));
		return [...ids].sort();
	}, [core.httpClients]);
	...
	return (
		<>
			<PageHead title="HTTP dependencies">
				Every outbound call that goes through <code>AxiosService</code> — D03 factories{' '}
				<code>this.get</code> / <code>this.getMany</code>, SAP/IKM/PNS{' '}
				<code>this.axios.post</code>, and the rest. A use case is listed only when its entry
				method actually reaches that client method.
			</PageHead>
			...
			<div className="toolbar">
				...
				>
					...
					{systems.map(id => (
						<option key={id} value={id}>
							{core.systems.find(s => s.id === id)?.title ?? id}
						</option>
					))}
				</select>
				<select className="select" value={verb} onChange={e => setParam('verb', e.target.value)}>
					<option value="all">Any verb</option>
					{(['GET', 'POST', 'PUT', 'PATCH', 'DELETE'] as HttpVerb[]).map(v => (
				...
				<span className="dimmer" style={{ fontSize: 12.5, marginLeft: 'auto' }}>
					{clients.length.toLocaleString()} of {(core.httpClients ?? []).length.toLocaleString()}
				</span>
			</div>
			...
			<PageHead
				...
				crumbs={
					<>
						...
						<span>/</span> <span className="mono">{client.system}</span>
					</>
				}
				actions={<SourceLink source={client.source} label="open in GitHub" />}
			>
				<span className="mono">{client.className}</span> calls <SystemLink id={client.system} />
				{client.baseUrlRef ? (
					<>
						{' '}
						...
						<tbody>
							{client.operations.map((op, i) => (
								<tr key={`${op.name}:${op.httpMethod}:${op.path}:${i}`}>
									...
									<td>
										<Badge tone={VERB_TONE[op.httpMethod]}>{op.httpMethod}</Badge>
									</td>
									<td className="mono" style={{ fontSize: 12.5 }}>
										/{op.path}
									</td>
									<td className="dimmer" style={{ fontSize: 12.5 }}>
										{op.via === 'factory' ? 'this.get / this.post' : 'this.axios.*'}
									</td>
								</tr>
							))}
						</tbody>
