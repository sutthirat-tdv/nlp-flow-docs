  ...
  return (
    <section className="section">
      <h2 style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
        ...
        {actions ? (
          <span style={{ marginLeft: "auto", fontSize: 13 }}>{actions}</span>
        ) : null}
      </h2>
      {children}
    </section>
...
export function UseCaseLink({ id }: { id: string }) {
  ...
  if (!useCase)
    return <span className="mono dimmer">{id.split(":")[1] ?? id}</span>;
  return (
...
export function HttpClientLink({ id }: { id: string }) {
  ...
  return (
    <Link to={`/dependencies/${encodeURIComponent(id)}`}>{client.name}</Link>
  );
}
...
export function FlowLink({
  ...
  return (
    <Link to={`/flows/${encodeURIComponent(flowId)}`}>
      {children ?? flow.title}
    </Link>
  );
}
