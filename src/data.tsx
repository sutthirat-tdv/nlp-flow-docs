...
export function DataProvider({ children }: { children: ReactNode }) {
	...
	const loadSchemas = useCallback(
		(repoId: string) => {
			...
			if (!promise) {
				promise = fetch(`${DATA_BASE}/schemas.${repoId}.json`).then(r => r.json());
	...
	const loadFlows = useCallback(
		(repoId: string) => {
			...
			if (!promise) {
				promise = fetch(`${DATA_BASE}/flows.${repoId}.json`).then(r => r.json());
	...
	const value = useMemo<DataContextValue | null>(() => {
		...
		return {
			...
			loadSchema,
			loadFlow,
			repoTitle: (repoId) => (repoId ? indexes.repoById.get(repoId)?.title ?? repoId : '—'),
		};
